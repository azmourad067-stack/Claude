...
APP_NAME = "HorseProno Quinté V4.4 — Winner Top 3"
PMU_BASE_URL = "https://online.turfinfo.api.pmu.fr/rest/client/1"
REQUEST_TIMEOUT = 25
HEADERS = {
    "User-Agent": "HorsePronoQuinteV44/4.4 (+https://streamlit.io)",
...
st.title("🏇 HorseProno Quinté V4.4 — Winner Top 3")
st.caption(
    "Le programme PMU complet de la journée est chargé automatiquement. "
    "V4.4 conserve exactement le Top 7 V4.2/V4.3 puis, en PLAT, réordonne ces 7 chevaux pour augmenter la présence du gagnant dans les 3 premières propositions. "
    "Le détecteur outsider V4.3 reste en shadow mode."
)
...
def run_prediction(history: pd.DataFrame, race: pd.DataFrame) -> pd.DataFrame:
    ...
    return rank_quinte_v44(history, race, artifact, ranker, outsider)
...
try:
    ...
    if st.button("🏇 Analyser cette course", type="primary"):
        ...
        st.subheader("🎯 Sélection V4.4 — Top 7 + objectif gagnant Top 3")
        ...
        if str(ranked.iloc[0].get("v44_mode", "")) == "WINNER_TOP3_PLAT":
            top3 = ranked.head(3)
            st.success(
                "🏆 **V4.4 — Gagnant Top 3 :** "
                + " - ".join(top3["horse_number"].astype(int).astype(str).tolist())
                + "  ·  ordre spécialisé PLAT (Top7 inchangé)"
            )
            st.caption(
                "Score d'ordre V4.4 PLAT : 50 % modèle Top5 + 40 % LambdaMART + 10 % forme/contexte. "
                "Cette couche ne change aucun cheval du Top7."
            )
        else:
            st.info("V4.4 : hors PLAT, l'ordre V4.3 est conservé à l'identique.")
        ...
        st.caption(
            ...
            "V4.2/V4.3 construit la shortlist de 7; V4.4 optimise ensuite uniquement son ordre sur le PLAT."
        )
        ...
        display = ranked[
            [
                ...
                "v44_winner_score", "v44_original_rank", "v42_edge_score", "v43_outsider_probability", "v43_outsider_candidate",
        ...
        display["v44_original_rank"] = pd.to_numeric(display["v44_original_rank"], errors="coerce").astype("Int64")
        ...
        display = display.rename(
            columns={
                ...
                "horse_name": "Cheval",
                "shortlist_role": "Rôle",
                "v44_winner_score": "Score Winner V4.4",
                "v44_original_rank": "Ancien rang V4.3",
