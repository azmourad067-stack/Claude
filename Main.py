...
def call_openai_chat(messages, model="gpt-4o-mini", temperature=0.7):
    # messages is a list of dicts {role: 'user'/'assistant'/'system', content: '...'}
    if not st.session_state.openai_key:
        st.error("Aucun OPENAI_API_KEY fourni. Entrez-le dans la barre latérale.")
        return None
    try:
...
def transcribe_audio_file(uploaded_file):
    # Accepts WAV/MP3/OGG etc; convert to wav then send to OpenAI whisper if key present
    if not st.session_state.openai_key:
        st.error("Aucun OPENAI_API_KEY fourni. Entrez-le dans la barre latérale pour activer la transcription audio.")
        return None
    try:
        ...
        wav_io.seek(0)
        openai.api_key = st.session_state.openai_key
        resp = openai.Audio.transcribe("whisper-1", wav_io)
        return resp['text']
    except Exception as e:
...
def tts_and_play(text, lang='en'):
    try:
        ...
        mp3_io.seek(0)
        st.session_state.last_response_audio = mp3_io.read()
...
st.set_page_config(page_title="English Buddy — Practice English", layout='wide')
init()

# Sidebar
st.sidebar.title("Paramètres")
api_key_input = st.sidebar.text_input("OpenAI API Key (optionnel mais recommandé)", type='password', value=st.session_state.openai_key)
if api_key_input and api_key_input != st.session_state.openai_key:
    set_openai_key(api_key_input)

st.sidebar.markdown("---")
model_choice = st.sidebar.selectbox("Model (OpenAI)", options=["gpt-4o-mini", "gpt-4o", "gpt-3.5-turbo"], index=0)
temp = st.sidebar.slider("Temperature", 0.0, 1.0, 0.7)

st.sidebar.markdown("---")
st.sidebar.markdown("**Mode d'entrée vocal**")
st.sidebar.markdown("1) Enregistrer un audio puis téléverser (fonctionne tout de suite).\n2) Utiliser la reconnaissance vocale du navigateur: appuyez sur 'Start (Browser mic)' puis copiez le texte reconnu dans la zone de saisie. (la capture live est limitée par Streamlit sans composant personnalisé)")

# Layout
col1, col2 = st.columns([3, 1])

with col1:
    st.title("English Buddy — discute avec une amie")
...
with col2:
    ...
    if st.button('Réécouter dernier message (TTS)'):
        if st.session_state.history and st.session_state.history[-1][0]=='assistant':
            tts_and_play(st.session_state.history[-1][1])
        else:
            st.info('Aucun message assistant à lire.')
